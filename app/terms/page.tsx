...
export default function TermsPage() {
  ...
  const sections = t("terms.sections", { returnObjects: true }) as Record<string, {
    title: string;
  ...
  const section15 = t("terms.sections.section15", { returnObjects: true }) as {
    title: string;
    content: string;
  }
    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="w-full bg-gradient-to-b from-[#f7fafc] to-white py-12">
        <div className="mx-auto max-w-4xl px-4">
          <div className="mb-12">
            <p className="text-slate-700 leading-relaxed">{t("terms.hero.description")}</p>
          </div>
          ...
          {Object.values(sections).map((section, index) => (
            <div key={index} className="mb-10">
              ...
              <div className="ml-13 space-y-4">
                {Array.isArray(section.content) ? section.content.map((p: string, pIdx: number) => (
           ...
           <div className="mt-12 p-6 bg-brand-blue/5 rounded-xl border border-brand-blue/20">
            <h2 className="text-2xl font-bold text-slate-800 mb-4">{section15.title}</h2>
            <div className="space-y-2 text-slate-700">
               <p>
                <strong>{section15.content.split('\n')[0]}</strong>
              </p>
              <p>
                <a
                  href={`mailto:${section15.content.split('\n')[1].split(': ')[1]}`}
                  className="text-brand-blue hover:text-brand-navy font-medium"
                >
                  {section15.content.split('\n')[1]}
                </a>
              </p>
            </div>
          </div>
